# Contributing to go-uds-jsonrpc
...
- A Unix-like OS for the socket-based tests (Linux, macOS). Windows is supported at runtime but the integration tests assume Unix sockets.

### Setup

```bash
git clone https://github.com/floatpane/go-uds-jsonrpc.git
cd go-uds-jsonrpc
go mod tidy
```
...
```
feat(server): add OnConnect hook before first ReceiveMessage
fix(transport): release write mutex on Encode error
docs: document SIGHUP-driven reload pattern
```
...
- A minimal reproducer (a `Server`/`Conn` snippet, the message bytes that trigger the bug, or a failing test)
