...
stateDiagram-v2
    [*] --> Starting
    Starting --> AlreadyRunning: IsRunning() == true
    AlreadyRunning --> [*]
    Starting --> Serving: PID written, socket listening (0700)
    Serving --> Serving: SIGHUP → reload + Broadcast(ConfigReloaded)
    Serving --> ShuttingDown: SIGINT / SIGTERM cancels ctx
    ShuttingDown --> [*]: close listener · RemovePID · unlink socket
```
